...
SELECT    -- main querry
       category_name
  FROM tbl_category
  WHERE category_code = (SELECT category_code FROM tbl_menu WHERE menu_name = '열무김치라떼');   --   sub querry
...
SELECT
  ...
  FROM tbl_menu A
 WHERE
       menu_price = (SELECT MAX(B.menu_price) FROM tbl_menu B WHERE B.category_code = A.category_code);
...
SELECT
  ...
  FROM tbl_menu A
  WHERE
        menu_price > (SELECT AVG(B.menu_price) FROM tbl_menu B WHERE B.category_code=A.category_code);
