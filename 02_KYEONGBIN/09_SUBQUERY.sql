...
FROM
    tbl_category
WHERE
    category_code = (SELECT category_code -- 서브쿼리
                     FROM tbl_menu
...
WHERE
 ...
 -- 여기서는 보통 category_code 와 (가로 안의 SELECT category_code 를 같게 입력)
...
-- 서브쿼리가 다중행을 반환하는 경우 '=' 연산자를 사용할 수 없다. IN을 사용하는 거 같음
SELECT *
FROM
    tbl_menu
WHERE
...
FROM
    tbl_menu
WHERE
    menu_price >= ALL(SELECT menu_price FROM tbl_menu); # 최대값보다 크거나 같으면 참
...
FROM
    tbl_menu a
WHERE
    menu_price = ALL (SELECT MAX(b.menu_price)
                       FROM tbl_menu b
...
FROM
    tbl_menu a
WHERE
    menu_price > ALL (SELECT AVG(b.menu_price)
                       FROM tbl_menu b
                       WHERE b.category_code = a.category_code);
...
# 필요한 내용을 그때만 딱 쓰고 그 후 안쓸 때 사용한다고 함...
