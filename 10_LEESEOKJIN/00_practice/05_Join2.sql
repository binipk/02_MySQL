...
SELECT EMP_ID AS                사번,
       EMP_NAME                 사원명,
       TIMESTAMPDIFF(YEAR, STR_TO_DATE((CASE
                                            WHEN SUBSTRING(EMP_NO, 8, 1) IN ('1', '2', '5', '6')
                                                THEN CONCAT('19', SUBSTRING(EMP_NO, 1, 6))
                                            WHEN SUBSTRING(EMP_NO, 8, 1) IN ('3', '4', '7', '8')
                                                THEN CONCAT('20', SUBSTRING(EMP_NO, 1, 6)) END), '%Y%m%d'),
                     CURDATE()) 나이,
       DEPT_TITLE               부서명,
       JOB_NAME                 직급명
FROM employee e
...
SELECT EMP_NAME   사원명,
       ...
       DEPT_TITLE 부서명
FROM employee e
         JOIN empdb.department d on e.DEPT_CODE = d.DEPT_ID
         JOIN empdb.job j on e.JOB_CODE = j.JOB_CODE
...
FROM employee e
         JOIN empdb.job j on e.JOB_CODE = j.JOB_CODE
         JOIN empdb.sal_grade sg on e.SAL_LEVEL = sg.SAL_LEVEL
...
SELECT EMP_NAME      사원명,
       ...
       NATIONAL_NAME 국가명
FROM employee
         JOIN empdb.department d on employee.DEPT_CODE = d.DEPT_ID
