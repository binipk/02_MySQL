...
SELECT FIND_IN_SET('바나나', '사과,딸기,바나나'); -- 리스트는 칸 띄우면 안됨
...
SELECT FORMAT(123412341234.51614,3);
...
SELECT
      INSERT(menu_name, INSERT(menu_name, '쥬스'), 2, 'JUICE')
  FROM
      tbl_menu
 WHERE
      menu_name LIKE '%쥬스%';
...
-- 881122-1******
SELECT LPAD('왼쪽', 6, '@'), RPAD('오른쪾', 6, '@'), RPAD('881122-1', 14, '*');
...
SELECT  REVERSE('stressedA');

-- SPACE(길이)
-- SPACE: 길이만큼의 공백을 반환
SELECT CONCAT('제 이름은', SPACE(5), '이고, 나이는 ', SPACE(3), '세입입니다. ') 나이소개;
...
SELECT ROUND(1234.56, 1), ROUND(1234.567, 2), TRUNCATE(1234.56, 1), TRUNCATE(-0.65, 0);
...
-- 년월일은 더하거나 뺄 수 없다.
SELECT ADDTIME('2024-03-21 15:19:00', '3:0:1'), SUBTIME('2024-03-21 15:19', '4:0:1');
...
-- 날짜 또는 시간에서 연, 월, 일, 시, 분, 초, 밀리초(1/1000)를 추출
...
-- CAST (expression AS 데이터형식 [(길이)]) // 캐스팅
