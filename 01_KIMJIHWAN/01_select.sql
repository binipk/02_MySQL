...
ORDER BY : 정렬한 컬럼을 기준으로 오름(ASC / default)/내림차순(DESC) 지정
*/
...
SELECT
    ...
    , category_name
  FROM tbl_menu
  JOIN menudb.tbl_category tc on `tc`.category_code = tbl_menu.category_code

-- -------------------------------------------------------------------------
# from절 없는 select 해보기
SELECT 7 + 3;
SELECT 10 * 2;
SELECT 6 % 3, 6 % 4;    # SELECT
SELECT NOW();
SELECT CONCAT('유', ' ', '관순');

-- 메뉴 이름은 참치 쉐이크와 생갈치 쉐이크이고, 가격은 30,000원 입니다.
...
-- 리터럴(값) 선언한 답 그자체 (1, 2, true, false 등등)
...
-- 열이 여러 개인 DISTINCT    // 컬럼이 한 세트가 되서 중복 값을 제거한다.
SELECT
        DISTINCT
        category_code
      , orderable_status
