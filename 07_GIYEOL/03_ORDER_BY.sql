...
SELECT
    ...
    FROM
        tbl_menu
ORDER BY
    -- menu_price ASC; -- ASC는 기본값이며 오름차순 /DESC :내림차순
 menu_price DESC;
 ...
 -- 가격 순으로 내림차순으로 지정하고 가격이 같으면 메뉴 이름의 오름차순으로 정렬
 -- 컬럼 순서로 지정해서 정렬
 SELECT
     menu_code,  -- 1
     menu_name,  -- 2
 ...
 ORDER BY
    메뉴가격 DESC;
 -- ORDER BY 절을 사용해 연산결과로 결과 집합 정렬
...
CREATE DATABASE  empdb; -- 데이터 베이스 생성
show databases;
GRANT ALL PRIVILEGES ON empdb.* TO 'employee'@'%';
*/


-- NULL 오름차순 시 NULL값은 맨 위에 온다
-- 내림차순 시 NULL 값은 아래에 온다
SELECT
    category_code,
    category_name,
    ref_category_code
