...
SELECT
    menu_name,
    BIT_LENGTH(menu_name), CHAR_LENGTH(menu_name),LENGTH(menu_name)
    FROM
        tbl_menu;
--
...
-- order by를 사용해 특정 레코드를 끌어 올리는 용도로 사용
...
SELECT  FORMAT(123412341234.5678,3);
SELECT
    menu_code 코드,
...
select CONCAT('제 이름은',SPACE(5),'이고,나이는 ',SPACE(3),'세 이다') 자기소개;


-- SUBSTRING(문자열, 시작위치, 길이)
-- SUBSTRING: 시작 위치부터 길이만큼의 문자를 반환(길이를 생략하면 시작 위치부터 끝까지 반환)
SELECT SUBSTRING('아령하세요 반갑습니다.', 7, 6),
...
SELECT ADDTIME('2024-03-21 15:19:00','3:0:1'), ADDTIME('2024-03-21 15:19:00', '4:0:1');
...
SELECT EXTRACT(DAY_HOUR FROM NOW()); -- HOUR  랑 DAY_HOUR는 동일
...
select '1' + '2';
select  concat(menu_price, '원') from tbl_menu ; -- menu_price 가 문자로 변환
...
SELECT  IFNULL(null,'hello world'), IFNULL('HHHH','HELLO WORLD');

 -- IF + ISNULL
 SELECT  IF(ISNULL(null),'이름없음','이름있음')as NAME;
 SELECT  IF(ISNULL(1),'이름없음','이름있음')as NAME;
...
*/
 -- sum 해당되는 값들의 총 합을 구함
 -- 해당 값이 null 인 행은 제외하고 합계를 구함
select menu_price from tbl_menu;
select sum(menu_price) from tbl_menu;

select category_code from tbl_menu;
select sum(category_code) from tbl_menu;


-- 2. avg 평균
-- 해당 컬럼 값들의 평균을 구하는 함수
-- 내부적으로 합계와 count 정보로 연산함

 select  avg(menu_price) from tbl_menu;
 -- count *
 -- 테이블 조건에 맞으면 행의 갯수 반환, *는 모든 행을 의미, 행이 존재하면 1로 카운트
  select
     count(*) -- null값 포함
  select
     count(카테고리 테이블) -- null 값 제외하고 카운트
...
--
select menu_name ,count(*) as '이름' from tbl_menu group by menu_name;
