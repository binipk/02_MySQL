...
# 함수에서는 크게 두 가지 유형으로 나뉜다.
# 첫번째는 단일행처리 함수
# 단일행 함수와 기본함수를 넣으면 결과가 안나온다? 일반적으로 섞어서 사용 불가
# 단일행 함수는 행 마다 적용, 그룹함수는 여러개를 하나로 묶어서 적용
...
SELECT
        INSTR('사과딸기바나나', '딸기'), # 찾는 문자가 앞에서부터 세번째이기 때문에 결과는 3
        LOCATE('딸기', '사과딸기바나나');
SELECT
    INSTR('사과딸기바나나', '바나나'),
    LOCATE('바나나', '사과딸기바나나');
...
-- 주민번호 첫째자리 말고 나머지 별표로 할때 위와 같은 함수를 사용한다.
-- 881122 - 1******

SELECT LPAD('왼쪽', 6, '@'), RPAD('오른쪽', '6', '@'), RPAD('881122-1', '14', '*'); -- 6은 문자의 전체길이
...
SELECT REPLACE('마이SQL', '마이', 'My'); # 마이를 찾고 My로 변경
...
SELECT
        ...
        RPAD(SUBSTRING('881122-1234567', 1, 8), 14, '*'), -- 점까지 6개
        SUBSTRING('안녕하세요 반갑습니다.', 7); -- 뒤에 없으면 7부터 끝까지
#        RPAD('881122-1', 14, '*');
...
SELECT NOW(), ADDDATE(NOW(), 1), SUBDATE(NOW(), 1); # 지금, 하루 뒤, 하루 전
SELECT ADDDATE('2025-03-21', INTERVAL 30 DAY); # 30일 후에 결과
SELECT ADDDATE('2025-03-21', INTERVAL 30 DAY), ADDDATE('2025-03-21', INTERVAL 5 MONTH); # 30일 후에 결과, 5개월 후 결과
SELECT SUBDATE('2025-03-21', INTERVAL 30 DAY), SUBDATE('2025-03-21', INTERVAL 5 MONTH); # 30일 전에 결과, 5개월 전 결과
...
-- 날짜 또는 시간에서 연, 월, 일, 시, 분, 초, 밀리초(1000분의 1초)를 추출
...
-- IF +  ISNULL
SELECT IF(ISNULL(null), '이름없음', '이름있음') AS NAME; # null이 아니라 다른값이었는데 생각이 안남..
...
SELECT
        menu_name,
        IF(orderable_status = 'Y', '주문 가능', '주문 불가') orderable_status # Y면 주문 가능, 아니면 주문 불가
  FROM
        tbl_menu
...
SELECT SUM(ref_category_code) FROM tbl_category; -- NULL 제외 합계
...
-- *는 모든 컬럼(하나의행)을 의미힌다. 행이 존재하면 1로 카운트
SELECT
        COUNT(*), -- NULL 포함 COUNT
...
# SELECT COUNT(*), category_code FROM tbl_menu; # 오류가 나오는 이유 다시 해보기