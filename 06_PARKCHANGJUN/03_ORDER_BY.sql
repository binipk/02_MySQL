...
SELECT
  ...
  FROM
        tbl_menu
 ORDER BY
        menu_price; -- default값이 오름차순이라 위에 값이랑 같은 결과가 나온다.
...
SELECT
        menu_code AS "메뉴 코드",
        menu_name "메뉴명",
        menu_price 메뉴가격 -- 띄어쓰기가 있을때 따옴표가 있어야 함.
  FROM
        tbl_menu
 ORDER BY
        메뉴가격 DESC;
# SELECT 절은 사칙연산이 가능하고 결과를 가지고 정렬할 수 있다.

-- ORDER BY절을 사용하여 연산결과로 결과 집합 정렬
SELECT
        menu_code 메뉴코드,
        menu_name 메뉴명,
