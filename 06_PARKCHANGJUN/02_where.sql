...
-- 같지 않을 연산자와 함께 WHERE절 사용 (Y랑 같지 않은 N만 나오게 됨)
SELECT
        menu_name,
        menu_price,
        orderable_status
...
-- 메뉴테이블에서 주문여부가 'Y'이면서, 카테고리코드가 10인 메뉴목록을 조회
SELECT
        menu_code
       ,menu_price
       ,category_code
...
SELECT 0 OR 0;
SELECT 1 OR NULL, NULL OR 1, NULL OR 0, 0 OR NULL, NULL OR NULL;

-- 메뉴테이블에서 주문여부가 'Y'이거나, 카테고리코드가 10인 메뉴목록을 조회
SELECT
        menu_code,
        menu_price,
        category_code,
...
-- NOT을 붙히면 부정(10000과 25000사이가 아니라 그 외)
SELECT
        menu_name,
        menu_price,
        category_code
...
# WHERE
#     IFNULL(ref_category_code, 0) = 0;
# where - 조건에 부합하는것만 걸러내겠다.
# 예를 들어, 연봉이 바뀌면 일일이 WHERE(조건)을 바꾸면 번거롭기 때문에 SELECT문에서 조건을 만든다.
...
-- 위에 IFNULL, COALESCE는 같은 결과를 나타낸다.
...
SELECT
  ...
  FROM
        tbl_category
 WHERE
        ref_category_code IS NOT NULL; -- NULL 빼고 나머지
