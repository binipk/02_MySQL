...
-- Table도 객체, ex) CREATE Table, CREATE User 여기에 있는 Table, User도 객체이다.
SHOW VARIABLES LIKE 'autocommit';
SET autocommit = OFF; # 메뉴얼로 바꾸겠다. 자동커밋 안하겠다라는 뜻
...
ROLLBACK; # CREATE 명령어 즉, DDL 명령어를 사용하면 이후에는 롤백이 안된다. DROP으로 지울 수 있음.
...
-- 테이블 구조 확인
DESCRIBE product1;
DESC product1; # 여기서 DESC는 내림차순이 아니라 DESCRIBE의 약어이다.
...
# AUTO INCREMENT가 있으면 null값을 넣을 수 있다.
...
SELECT * FROM tbl_rent;
# Default를 걸어놓으면 하나만 써도 INSERT가 가능하다.

# AUTO_INCREMENT
-- 밑에 예시서 PRIMARY KEY가 없으면 사용 불가.
-- INSERT시 PK 컬럼에 자동으로 번호를 발생(중복되지않게)시켜 저장할 수 있다.
-- PK컬럼이 아니 컬럼은 사용할 수 없다.
...
DROP TABLE IF EXISTS product1; # 만약에 존재하면 삭제
