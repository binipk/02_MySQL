...
-- LENGTH : 할당된 BYTE 크기 (1byte = 8bit)

SELECT menu_name, BIT_LENGTH(menu_name), CHAR_LENGTH(menu_name) FROM tbl_menu;
...
-- FIELD: 찾을 문자열 위치 반환
-- ELT(Extract LisT): 해당 위치의 문자열 반환
...
-- 문자열 리스트 : '사과,딸기,바나나' (콤마 뒤 공백 허용X)
SELECT FIND_IN_SET('바나나', '사과,딸기,바나나');

# 특정 레코드를 뜬금없이 맨 위로 올림
# ORDER BY에서 특정 레코드를 끌어올리기 위한 용ㄷ도로 사용할 수 있다.
...
-- FORMAT(숫자, 소수점 자릿수)
-- FORMAT: 1000단위마다(3자리) 콤마 표시룰 해주며 소수점 아래에서 반올림

SELECT FORMAT(123412341234.5165465, 3);
SELECT menu_code, FORMAT(menu_price, 0) 단위 FROM tbl_menu;

-- INSERT(기준 문자열, 위치, 길이, 삽입할 문자열)
-- INSERT: 기준 문자열의 위치부터 길이만큼을 지우고 삽입할 문자열을 끼워 넣는다.
SELECT INSERT('내 이름은 아무개 입니다.', 7, 3, '길동길동');

# 조회한 결과를 조작한 것. 원본은 바뀌지 않는다.
SELECT # 메뉴 네임에서, INSTR로 쥬스가 시작하는 위치를 찾고, 2글자 삭제-> JUICE 삽입
...
-- LPAD: 문자열을 왼쪽으로 늘려 길이로 만든 후에 빈 곳을 문자열로 채운다.
-- RPAD: 문자열을 오른쪽으로 늘려 길이로 만든 후에 빈 곳을 문자열로 채운다.

SELECT LPAD('왼쪽', 6, '@'), RPAD('왼쪽', 6, '@');
...
-- SUBSTRING(문자열, 시작위치, 길이) -> 1based
-- SUBSTRING: 시작 위치부터 길이만큼의 문자를 반환(길이를 생략하면 시작 위치부터 끝까지 반환)
SELECT
       SUBSTRING('안녕하세요 반갑습니다.',7,6)
     , RPAD(SUBSTRING('881122-1234567',1,8),14,'*') #881122-1 뒤에 *붙여서 14자리 만들기
...
-- TRUNCATE(숫자, 소수점이하 자리수): 버림값 반환. 소수점이하 자리수는 필수.
-- ROUND: 반올림값 반환 ROUND(~~,2) 2째자리까지 반환

SELECT CEILING(1234.56), FLOOR(1234.56), FLOOR(-0.65);
SELECT ROUND(1234.56), ROUND(1234.567, 2), TRUNCATE(1234.56,1), TRUNCATE(-0.56,0);
...
SELECT ADDTIME('2025-03-21 15:19:00', '2:40:58'), SUBTIME('2025-03-21 15:19:00', '2:40:58');
...
SELECT
       DATEDIFF('2025-02-21',NOW()), TIMEDIFF('18:00:00', CURTIME());
SELECT
       DATEDIFF('2025-03-21', '2025-09-09');
...
-- 데이터 형식으로 가능한 것은
-- BINARY, CHAR, DATE, DATETIME, DECIMAL, JSON,
...
SELECT 5 > '6MAY';
SELECT 3 > '6MAY', '6MAY' + 1;
SELECT ADDDATE('2025-03-11', 1); -- 날짜형으로 바뀔 수 있는 문자는 DATE형으로 변환
...
SELECT IFNULL(null, 'HELLO WORLD'), IFNULL('ㅋ', 'HELLO WORLD');

-- IF + ISNULL
SELECT IF(ISNULL(null), '이름없음', '이름있음') as NAME;
SELECT IF(ISNULL('길동'), '이름없음', '이름있음') as NAME;

-- IF
-- IF(조건, '조건이 일치할 경우의 값', '조건이 일치하지 않을 경우의 값')
SELECT
       menu_name
     , IF(orderable_status='Y', '주문가능', '주분 불가') orderable_status
  FROM tbl_menu;
...
SELECT
    ...
    CASE
        ...
        WHEN menu_price BETWEEN 5000 AND 9999 THEN '적당한 거'
        WHEN menu_price BETWEEN 10000 AND 200000 THEN '비싼 거'
        ELSE '알 수 없음'
...
-- 해당 칼럼의 값이 null인 행은 제외하고, 합계룰 구한다.
...
-- 내부적으로 합계와 COUNT정보를 갖고 연산
SELECT AVG(ref_category_code) FROM tbl_category;

-- 3. COUNT
-- 테이블에서 조건을 만족하는 행의 갯수를 반환하는 함수
-- *는 모든 컬럼을 의미한다. 행이 존재하면 1로 카운트
SELECT

       COUNT(*) # 널포함 카운트
     , COUNT(ref_category_code) # 널미포함 카운트
  FROM
      tbl_category;
